...
触发语「银行晨报」「分析今天的 brief」「读今日背景材料」任一即可。这是**消费流程，不重跑流水线**：

1. 取 `workspace/brief/` 下最新一份 `YYYY-MM-DD.md` 和最新主报告 JSON，两份产物日期可能不同（brief 是昨日 21:30、主报告是昨日 22:30、行情数据日更早一天），开头先各自声明日期
2. 检查 brief 的「缺失源/数据陈旧」标注，缺口在分析开头显式声明，不得静默使用陈旧数据
3. 按 `references/bank_framework.md` §7.5 四段框架输出：**节奏**(L1温度×ETF份额资金流) → **结构**(L2高分池×公告互证) → **主题**(新闻聚类×对应模型因子) → **风险**(素材盲区)
4. 宏观解读遵循 §7 传导方程与三层回答法（账面利润/真实ROE/股价分层，不混答）；素材层信息永不改写评分与档位
...
L2 个股基础分 = 盈利30% + 资产质量25% + 成长15% + 资本充足10% + 估值吸引力20%
    ...
    边际因子(v1.2): NIM同比变化(盈利维度10%子权重) · 不良率同比变化(质量维度10%子权重, 低优)
数据可信度(v1.2, 仅标注不改分): 覆盖度+财报新鲜度+是否回退旧报告期 → 高/中/低
期次滞后(v1.2, ※徽标): 报告期落后于全池主流期的行, 中报季流量指标与主截面不可比
...
| ETF份额(沪) | akshare `fund_etf_scale_sse(date)` | 逐日全市场, 盘后约19:00发布, SQLite逐日累积 |
| ETF份额(深) | akshare `fund_scale_daily_szse(起,止)` | 区间一次拉取, 同上 |
...
- **ETF份额列为空/累计变化为空** → 份额数据盘后约 19:00 发布，盘中运行取到的是前一交易日；累计变化自本地落库起累积，冷启动期(前5个交易日)为空属正常。
